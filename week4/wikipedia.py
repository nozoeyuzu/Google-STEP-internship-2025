...
class Wikipedia:
    ...
    def find_shortest_path(self, start, goal):
        ...
        while queue:
            ...
            if current_id == goal_id:
                ...
                # ページタイトルを取得
                titles = [self.titles[page_id] for page_id in path]
                print("->".join(titles))
                return
    ...
    def find_most_popular_pages(self):
        ...
        # 自分がどの隣接ノードからリンクされているかを保持
        neighbor_node = {page_id: [] for page_id in self.titles.keys()}
        for page_id, links in self.links.items():  # リンクされているページを取得
            for link in links:
                neighbor_node[link].append(page_id)

        # ページランクの計算
        for i in range(10000):  # 最大10000回の反復
            for page_id in self.titles:
                ...
                for neighbor in neighbor_node[page_id]:
                    ...
                    new_page_ranks[page_id] += damping_factor * (
                        old_page_ranks[neighbor] / len(self.links[neighbor])
                    )

            # 収束判定とold_page_ranksの更新
            diff = max(
                abs(new_page_ranks[page_id] - old_page_ranks[page_id])
            ...
            print(i, f"{diff:.6f}")  # デバッグ用に差分を表示
            if diff < 1e-6:  # 十分に収束したら終了
                break

        # ページランクの高い上位10ページを取得
        top10_pages = sorted(old_page_ranks.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
