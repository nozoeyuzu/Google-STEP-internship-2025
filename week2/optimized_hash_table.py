...
class HashTable:
    ...
    def put(self, key, value):
        ...
        #再ハッシュが必要かチェックし、必要であれば実行
        new_bucket_size = self.rehash_check()
    ...
    def rehash_check(self):
        if self.item_count >= self.bucket_size * 0.7:
            ...
            new_bucket_size = self.bucket_size
    ...
    def rehash(self, new_bucket_size):
        ...
        for i in range(len(current_buckets)):
            item = current_buckets[i]
            while item:
                next_item = item.next #Itemは新たにテーブルに追加する要素
                new_index = calculate_hash(item.key) % new_bucket_size
                item.next = new_buckets[new_index]  #新しいバケットにアイテムを追加
                new_buckets[new_index] = item  #新しいバケットの先頭に設定
                item = next_item  #次のアイテムに移動
        self.buckets = new_buckets  #新しいバケットを設定
